...
public class AgentskiCentarREST implements AgentskiCentarRESTRemote {
	...
	public String test() {
		String retVal = "";
		retVal += "Trenutni cvor: " + database.getAgentskiCentar().getAlias() + " "
				+ database.getAgentskiCentar().getAddress() + "\n\n";
		retVal += "\nAgentski centri: \n";
		...
		retVal += "\nAktivni agenti: \n";
		for (AID aid : database.getActiveAgentAIDS()) {
			retVal += aid.toString() + "\n";
		}
	...
	public List<AID> getActiveAgents() {
		return database.getActiveAgentAIDS();
	}
			...
			for (AgentskiCentar ac : database.getAgentskiCentri()) {
				...
				if (!ac.getAddress().equals(database.getAgentskiCentar().getAddress())) {
					ResteasyWebTarget target = client
							.target("http://" + ac.getAddress() + ":8080/AgentiWeb/rest/agentskiCentar/agents/running");
		...
		for (AgentskiCentar ac : database.getAgentskiCentri()) {
			if (ac.getAddress().equals(database.getAgentskiCentar().getAddress())) {
				...
				ResteasyWebTarget target = client.target("http://" + ac.getAddress()
						+ ":8080/AgentiWeb/rest/agentskiCentar/agents/running/remove/" + aid + "/" + hostName);
	...
	/**
	 * Nov ne-master čvor kontaktira master čvor koji ga registruje; Master čvor
	 * javlja ostalim ne-master čvorovima da je nov ne-master čvor ušao u mrežu;
	 * Master čvor dostavlja spisak ostalih ne-master čvorova novom ne-master
	 * čvoru;
	 */
	@POST
	@Path("/node")
	...
	public List<AgentskiCentar> register(AgentskiCentar agentskiCentar) {
		...
		try {
			if (database.isMaster()) {
				...
				// Zahtev za spisak podrzavanih agenata od novog cvora (@GET
				// /agents/classes)
				ResteasyClient client = new ResteasyClientBuilder().build();
				System.out.println(
						"http://" + agentskiCentar.getAddress() + ":8080/AgentiWeb/rest/agentskiCentar/agents/classes");
				ResteasyWebTarget target = client.target(
						"http://" + agentskiCentar.getAddress() + ":8080/AgentiWeb/rest/agentskiCentar/agents/classes");
				Response response = target.request(MediaType.APPLICATION_JSON).get();
				ArrayList<AgentType> podrzavaniAgenti = (ArrayList<AgentType>) response
						.readEntity(new GenericType<List<AgentType>>() {
						});
				database.addSviTipoviAgenata(podrzavaniAgenti);

				// Salje se zahtev za register svim cvorovima osim master cvoru
				// i novom cvoru (@POST /node)
				for (AgentskiCentar ac : database.getAgentskiCentri()) {

					if (!ac.getAddress().equals(database
							.getMasterIP()) /*
											 * && !ac.getAddress().equals(
											 * agentskiCentar.getAddress()
											 */) {
						target = client
								.target("http://" + ac.getAddress() + ":8080/AgentiWeb/rest/agentskiCentar/node");
				...
				// Salje se spisak novih tipova agenata svim cvorovima osim
				// masteru (@POST /agents/classes)
				for (AgentskiCentar ac : database.getAgentskiCentri()) {
					if (!ac.getAddress().equals(database.getMasterIP())) {
						target = client.target(
								"http://" + ac.getAddress() + ":8080/AgentiWeb/rest/agentskiCentar/agents/classes");
						response = target.request()
								.post(Entity.entity(database.getSviTipoviAgenata(), MediaType.APPLICATION_JSON));
					}
				}
				...
				target = client.target(
						"http://" + agentskiCentar.getAddress() + ":8080/AgentiWeb/rest/agentskiCentar/agents/running");
	...
	/**
	 * Master čvor dostavlja spisak novih tipova agenata (ukoliko ih ima)
	 * ostalim nemaster čvorovima; Master čvor dostavlja spisak tipova agenata
	 * novom ne-master čvoru koje podržava on ili neki od ostalih ne-master
	 * čvorova;
	 */
	@POST
	@Path("/agents/classes")
	...
	/**
	 * Master čvor dostavlja spisak pokrenutih agenata novom ne-master čvoru
	 * koji se nalaze kod njega ili nekog od preostalih ne-master čvorova;
	 */
	...
	/**
	 * Master čvor javlja ostalim ne-master čvorovima da obrišu čvor koji nije
	 * uspeo da izvrši handshake, kao i sve tipove agenata koji su potencijalno
	 * dostavljeni ostalim čvorovima. Ova operacije se takođe treba eksplicitno
	 * pokrenuti kada se neki čvor priprema za gašenje i želi da se odjavi iz
	 * klastera. Prilikom gašenja čvora treba pogasiti i sve agente koji trče na
	 * datom čvoru;
	 */
	@DELETE
	@Path("/node/{alias}")
		...
		if (database.isMaster()) {
			for (AgentskiCentar agentskiCentar : database.getAgentskiCentri()) {
				if (!agentskiCentar.getAlias().equals(database.getAgentskiCentar().getAlias())) {
					ResteasyClient client = new ResteasyClientBuilder().build();
					ResteasyWebTarget target = client.target("http://" + agentskiCentar.getAddress()
							+ ":8080/AgentiWeb/rest/agentskiCentar/node/" + alias);
					target.request().delete();
				}
			}
		}
	...
	/**
	 * Ukoliko se desi da čvor ne odgovori zahtev se izvršava još jednom i
	 * ukoliko čvor ni tada ne odgovori smatra se da je ugašen i javlja se
	 * ostalim čvorovima da izbace zapis o ugašenom čvoru.
	 */
	@GET
	@Path("/node")
	@Produces(MediaType.TEXT_PLAIN)
